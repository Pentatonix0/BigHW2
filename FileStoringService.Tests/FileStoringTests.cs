...
namespace FileStoringService.Tests
{
    public class FileStorageServiceTests : IDisposable
    {
        ...
        public async Task GetFileInfoAsync_ExistingFile_ReturnsFileInfo()
        {
            ...
            Assert.True(File.Exists(fileInfo.Path)); // Заменили fileInfo.Exists на File.Exists(fileInfo.Path)
            Assert.Equal(3, fileInfo.Size); // Заменили fileInfo.Length на fileInfo.Size
        }
